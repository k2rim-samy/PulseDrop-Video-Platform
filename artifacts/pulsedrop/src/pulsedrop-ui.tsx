...
function presetFormatLabel(formatId: string) {
  if (formatId === 'mp4:best') return 'MP4 · Best available';
  if (formatId === 'audio:best') return 'M4A · Best available';
  if (formatId.startsWith('mp4:')) return `MP4 · ${formatId.slice(4)}p`;
  return formatId.startsWith('audio:') ? `M4A · ${formatId.slice(6)}` : formatId;
}
...
function FormatChoice({ format, selected, onSelect }: { format: FormatOption; selected: boolean; onSelect: () => void }) {
  const audio = format.extension === 'm4a';
  const size = format.sizeMb > 0 ? `~${format.sizeMb} MB` : 'Size varies';
  return (
    <button type="button" onClick={onSelect} aria-pressed={selected} data-testid={`button-format-${format.formatId}`} className={`pd-focus relative w-full rounded-2xl border p-3.5 text-left transition-all duration-200 ${selected ? 'border-[hsl(var(--primary))] bg-[hsl(var(--primary)/.1)] shadow-[0_0_0_1px_hsl(var(--primary)/.18)]' : 'border-[hsl(var(--border))] bg-[hsl(var(--background)/.38)] hover:-translate-y-0.5 hover:border-[hsl(var(--primary)/.45)]'}`}>
      {format.recommended ? <span className="absolute -top-2 right-3 rounded-full bg-[hsl(var(--accent))] px-2 py-0.5 text-[9px] font-semibold tracking-wide">RECOMMENDED</span> : null}
      <div className="flex items-center gap-3">
        <div className={`grid h-10 w-10 shrink-0 place-items-center rounded-xl ${selected ? 'bg-[hsl(var(--primary))] text-[hsl(var(--primary-foreground))]' : 'bg-[hsl(var(--muted))] text-[hsl(var(--muted-foreground))]'}`}>{audio ? <AudioLines size={17} /> : <FileVideo size={17} />}</div>
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-2"><div className="truncate text-sm font-semibold">{format.label}</div>{selected ? <Check size={15} className="shrink-0 text-[hsl(var(--primary))]" /> : null}</div>
          <div className="pd-mono mt-1 text-[10px] text-[hsl(var(--muted-foreground))]">{format.quality} <span className="mx-1 opacity-40">·</span> {size}</div>
        </div>
      </div>
    </button>
  );
...
function Analyzer({ onCreated }: { onCreated: () => void }) {
  ...
  const availableFormats = analysis?.formats.filter((format) => format.formatId.startsWith(mode === 'audio' ? 'audio:' : 'mp4:')) ?? [];
  ...
  const submitAnalyze = (event: React.FormEvent) => {
    ...
    analyze.mutate({ data: { url: url.trim() } }, {
      onSuccess: (result) => {
        setAnalysis(result);
        setMode('video');
        const recommended = result.formats.find((format) => format.recommended && format.formatId.startsWith('mp4:'))
          ?? result.formats.find((format) => format.formatId.startsWith('mp4:'))
          ?? result.formats[0];
  ...
  const selectMode = (nextMode: 'video' | 'audio') => {
    setMode(nextMode);
    if (nextMode === 'audio') setSubtitles(false);
    const nextFormats = analysis?.formats.filter((format) => format.formatId.startsWith(nextMode === 'audio' ? 'audio:' : 'mp4:')) ?? [];
    setSelectedFormat(nextFormats.find((format) => format.recommended)?.formatId ?? nextFormats[0]?.formatId ?? '');
  };
  const submitDownload = () => {
    if (!analysis || !selectedFormat) return;
    create.mutate({ data: { url: analysis.sourceUrl, title: analysis.title, channel: analysis.channel, thumbnailUrl: analysis.thumbnailUrl, durationSeconds: analysis.durationSeconds, formatId: selectedFormat, mode, chapterMode, includeSubtitles: subtitles } }, {
      onSuccess: () => {
          ...
          <div className="grid gap-6 lg:grid-cols-[minmax(0,1.1fr)_minmax(340px,.9fr)]">
            <div>
              ...
              <div className="mt-6 rounded-xl bg-[hsl(var(--accent)/.23)] p-4"><div className="mb-1 flex items-center gap-2 text-[11px] font-semibold"><Sparkles size={13} /> Pulse recommendation</div><p className="text-sm leading-5 text-[hsl(var(--foreground)/.76)]">{analysis.smartRecommendation}</p></div>
               <div className="mt-6"><div className="mb-3 flex items-center justify-between"><div className="pd-kicker text-[hsl(var(--muted-foreground))]">01 / choose output</div><span className="pd-mono text-[10px] text-[hsl(var(--muted-foreground))]">MP4 or M4A</span></div><div className="grid grid-cols-2 gap-2">
                 <button type="button" onClick={() => selectMode('video')} aria-pressed={mode === 'video'} className={`pd-focus rounded-2xl border p-3 text-left transition-colors ${mode === 'video' ? 'border-[hsl(var(--primary))] bg-[hsl(var(--primary)/.1)]' : 'border-[hsl(var(--border))] hover:border-[hsl(var(--primary)/.45)]'}`} data-testid="button-mode-video"><div className="flex items-center gap-2 text-sm font-semibold"><FileVideo size={16} /> MP4 video</div><div className="mt-1 text-[10px] leading-4 text-[hsl(var(--muted-foreground))]">Picture + matched audio</div></button>
                 <button type="button" onClick={() => selectMode('audio')} aria-pressed={mode === 'audio'} className={`pd-focus rounded-2xl border p-3 text-left transition-colors ${mode === 'audio' ? 'border-[hsl(var(--primary))] bg-[hsl(var(--primary)/.1)]' : 'border-[hsl(var(--border))] hover:border-[hsl(var(--primary)/.45)]'}`} data-testid="button-mode-audio"><div className="flex items-center gap-2 text-sm font-semibold"><AudioLines size={16} /> M4A audio</div><div className="mt-1 text-[10px] leading-4 text-[hsl(var(--muted-foreground))]">Source bitrate preserved</div></button>
               </div></div>
            </div>
             <div><div className="mb-3 flex items-end justify-between gap-3"><div><div className="pd-kicker text-[hsl(var(--muted-foreground))]">02 / choose quality</div><p className="mt-1 text-xs text-[hsl(var(--muted-foreground))]">{mode === 'video' ? 'Every choice is delivered as an MP4.' : 'Choose the source audio bitrate.'}</p></div><span className="pd-mono text-[10px] text-[hsl(var(--muted-foreground))]">{availableFormats.length} options</span></div><div className="space-y-2">{availableFormats.map((format) => <FormatChoice key={format.formatId} format={format} selected={selectedFormat === format.formatId} onSelect={() => setSelectedFormat(format.formatId)} />)}</div><div className="mt-5 grid grid-cols-2 gap-2"><label className="flex cursor-pointer items-center gap-2 rounded-xl border border-[hsl(var(--border))] p-3 text-xs"><input type="checkbox" checked={chapterMode === 'split'} onChange={(event) => setChapterMode(event.target.checked ? 'split' : 'full')} className="accent-[hsl(var(--primary))]" data-testid="input-split-chapters" /> Split chapters</label>{mode === 'video' ? <label className="flex cursor-pointer items-center gap-2 rounded-xl border border-[hsl(var(--border))] p-3 text-xs"><input type="checkbox" checked={subtitles} onChange={(event) => setSubtitles(event.target.checked)} className="accent-[hsl(var(--primary))]" data-testid="input-subtitles" /> Include subtitles</label> : <div className="flex items-center rounded-xl border border-dashed border-[hsl(var(--border))] px-3 text-[10px] text-[hsl(var(--muted-foreground))]">Subtitles are video-only</div>}</div>{create.isError ? <div className="mt-3 rounded-xl border border-[hsl(var(--destructive)/.35)] bg-[hsl(var(--destructive)/.08)] px-3 py-2 text-xs text-[hsl(var(--destructive))]" data-testid="status-download-error">Couldn’t queue this file. Check the source and try again.</div> : null}<button type="button" onClick={submitDownload} disabled={create.isPending || !selected} className="pd-focus mt-3 flex h-12 w-full items-center justify-center gap-2 rounded-xl bg-[hsl(var(--secondary))] text-sm font-semibold text-[hsl(var(--secondary-foreground))] transition-transform hover:-translate-y-0.5 disabled:opacity-55" data-testid="button-start-download">{create.isPending ? <LoaderCircle size={16} className="animate-spin" /> : <DownloadIcon size={16} />} {create.isPending ? 'Queueing file' : `Download ${selected?.extension?.toUpperCase() ?? 'file'}`}</button></div>
          </div>
        </div> : null}
      </div>
    </section>
...
function DownloadRow({ download, onSelect, onDelete, onStart, onCancel }: { download: Download; onSelect: () => void; onDelete: () => void; onStart: () => void; onCancel: () => void }) {
  return (
    <div className="group grid gap-3 border-b border-[hsl(var(--border)/.75)] py-4 first:pt-1 last:border-0 sm:grid-cols-[minmax(0,1fr)_145px_96px] sm:items-center" data-testid={`row-download-${download.id}`}>
       <button type="button" onClick={onSelect} className="pd-focus flex min-w-0 items-center gap-3 text-left" data-testid={`button-open-download-${download.id}`}><div className="grid h-12 w-[76px] shrink-0 place-items-center overflow-hidden rounded-lg bg-[hsl(var(--muted))]">{download.thumbnailUrl ? <img src={download.thumbnailUrl} alt="" className="h-full w-full object-cover transition-transform duration-300 group-hover:scale-105" /> : <FileVideo size={18} className="text-[hsl(var(--muted-foreground))]" />}</div><div className="min-w-0"><div className="truncate text-sm font-medium">{download.title}</div><div className="mt-1 flex items-center gap-2 text-[11px] text-[hsl(var(--muted-foreground))]"><span className="truncate">{download.channel || 'YouTube source'}</span><span className="opacity-40">/</span><span className="pd-mono">{download.formatLabel}</span></div></div></button>
...
export function PresetsPage() {
  ...
  const [formatId, setFormatId] = useState('mp4:best');
  const [mode, setMode] = useState<'video' | 'audio'>('video');
  const [chapterMode, setChapterMode] = useState<'full' | 'split'>('full');
  const [subtitles, setSubtitles] = useState(false);
  ...
  return <><div className="mb-8"><div className="pd-kicker mb-2 text-[hsl(var(--primary))]">Remove decisions</div><h1 className="text-4xl font-semibold tracking-[-.07em]">Presets</h1><p className="mt-2 max-w-[520px] text-sm leading-6 text-[hsl(var(--muted-foreground))]">Name the setup you reach for most. Next time, your choice is already made.</p></div><div className="grid gap-5 lg:grid-cols-[.8fr_1.2fr]"><form onSubmit={save} className="pd-card h-fit rounded-2xl p-5"><div className="pd-kicker mb-4 text-[hsl(var(--muted-foreground))]">New preset</div><label className="mb-4 block text-xs font-medium">Preset name<input value={name} onChange={(event) => setName(event.target.value)} className="pd-focus mt-2 h-11 w-full rounded-xl border border-[hsl(var(--border))] bg-[hsl(var(--background)/.55)] px-3 text-sm outline-none focus:border-[hsl(var(--primary))]" placeholder="My clean archive" data-testid="input-preset-name" /></label><div className="mb-4 grid grid-cols-2 gap-2"><button type="button" onClick={() => { setMode('video'); setFormatId('mp4:best'); }} className={`rounded-xl border p-2.5 text-xs ${mode === 'video' ? 'border-[hsl(var(--primary))] bg-[hsl(var(--primary)/.1)]' : 'border-[hsl(var(--border))]'}`} data-testid="button-preset-video"><FileVideo size={14} className="mx-auto mb-1" />MP4 video</button><button type="button" onClick={() => { setMode('audio'); setFormatId('audio:best'); }} className={`rounded-xl border p-2.5 text-xs ${mode === 'audio' ? 'border-[hsl(var(--primary))] bg-[hsl(var(--primary)/.1)]' : 'border-[hsl(var(--border))]'}`} data-testid="button-preset-audio"><AudioLines size={14} className="mx-auto mb-1" />M4A audio</button></div><label className="mb-4 block text-xs font-medium">Quality<select value={formatId} onChange={(event) => setFormatId(event.target.value)} className="pd-focus mt-2 h-11 w-full rounded-xl border border-[hsl(var(--border))] bg-[hsl(var(--background)/.55)] px-3 text-sm outline-none focus:border-[hsl(var(--primary))]" data-testid="select-preset-quality">{mode === 'video' ? <><option value="mp4:best">Best available MP4</option><option value="mp4:1080">MP4 · 1080p</option><option value="mp4:720">MP4 · 720p</option><option value="mp4:480">MP4 · 480p</option></> : <><option value="audio:best">Best available M4A</option><option value="audio:192">M4A · up to 192 kbps</option><option value="audio:128">M4A · up to 128 kbps</option></>}</select></label><div className="mb-4 flex gap-4 text-xs"><label className="flex items-center gap-2"><input type="radio" checked={chapterMode === 'full'} onChange={() => setChapterMode('full')} data-testid="radio-preset-full" />Single file</label><label className="flex items-center gap-2"><input type="radio" checked={chapterMode === 'split'} onChange={() => setChapterMode('split')} data-testid="radio-preset-split" />Split chapters</label></div><label className={`mb-5 flex items-center gap-2 text-xs ${mode === 'audio' ? 'text-[hsl(var(--muted-foreground))]' : ''}`}><input type="checkbox" checked={subtitles} disabled={mode === 'audio'} onChange={(event) => setSubtitles(event.target.checked)} data-testid="input-preset-subtitles" /> Include subtitles <span className="text-[10px] opacity-70">{mode === 'audio' ? '(video only)' : ''}</span></label><button type="submit" disabled={create.isPending || !name.trim() || !formatId.trim()} className="flex h-11 w-full items-center justify-center gap-2 rounded-xl bg-[hsl(var(--secondary))] text-sm font-medium text-[hsl(var(--secondary-foreground))] disabled:opacity-50" data-testid="button-save-preset">{create.isPending ? <LoaderCircle size={15} className="animate-spin" /> : <Plus size={15} />} Save preset</button></form><div className="pd-card rounded-2xl p-5"><div className="mb-4 flex items-center justify-between"><div className="pd-kicker text-[hsl(var(--muted-foreground))]">Saved setups</div><span className="pd-mono text-xs text-[hsl(var(--muted-foreground))]">{query.data?.length ?? '—'}</span></div>{query.isLoading ? <div className="space-y-3">{[1, 2].map((i) => <div key={i} className="h-16 animate-pulse rounded-xl bg-[hsl(var(--muted))]" />)}</div> : query.isError ? <div className="py-12 text-center text-sm">Couldn’t load presets.</div> : query.data?.length ? <div className="space-y-2">{query.data.map((preset: Preset) => <div key={preset.id} className="flex items-center gap-3 rounded-xl border border-[hsl(var(--border))] p-3.5 transition-colors hover:bg-[hsl(var(--muted)/.45)]" data-testid={`card-preset-${preset.id}`}><div className="grid h-9 w-9 place-items-center rounded-lg bg-[hsl(var(--accent)/.45)]"><Gauge size={16} /></div><div className="min-w-0 flex-1"><div className="text-sm font-medium">{preset.name}</div><div className="pd-mono mt-1 text-[10px] text-[hsl(var(--muted-foreground))]">{presetFormatLabel(preset.formatId)} · {preset.chapterMode === 'split' ? 'split chapters' : 'single file'}{preset.includeSubtitles ? ' · subs' : ''}</div></div><button type="button" onClick={() => { if (window.confirm('Delete this preset?')) remove.mutate({ presetId: preset.id }, { onSuccess: () => void queryClient.invalidateQueries({ queryKey: getListPresetsQueryKey() }) }); }} className="pd-focus rounded-lg p-2 text-[hsl(var(--muted-foreground))] hover:bg-[hsl(var(--destructive)/.1)] hover:text-[hsl(var(--destructive))]" data-testid={`button-delete-preset-${preset.id}`}><Trash2 size={15} /></button></div>)}</div> : <div className="py-12"><EmptyState compact /></div>}</div></div></>;
}
